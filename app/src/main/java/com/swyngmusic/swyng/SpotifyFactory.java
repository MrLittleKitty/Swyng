...
public class SpotifyFactory
{
    ...
    public static String createPlaylist(AuthToken token, String userId, String playlistName)
    {
        try {
            ...
            return response.getBody().getObject().getString("uri");
        } catch (UnirestException e) {
            return null;
        } catch (JSONException e) {
            return null;
    ...
    public Track getTrack(AuthToken token, String userId, String playlistId)
    {
        try {
            ...
            JSONArray tracks = pagingObject.getJSONArray("items");
            if(tracks.length() <= 0)
                return null;

            JSONObject trackObj = tracks.getJSONObject(0);
